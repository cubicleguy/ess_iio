...
/*
* The epson-iiostream program is based on the Dummy IIO streaming example.
* Copyright(C) SEIKO EPSON CORPORATION 2020,2025. All rights reserved.
* Version 1.30
*
* Command line examples to run epson-iiostream with local backend: 
* > sudo ./epson-iiostream
* > sudo ./epson-iiostream -d epsonG354PDH0 -t epsonG354PDH0-dev0 -c 10
* > sudo ./epson-iiostream -d epsonG354PDH0 -t trigger0 -c 10
* > sudo ./epson-iiostream -d iio:device0 -t trigger0 -c 10
*
* To run epson-iiostream with network backend run iiod server first:
* > sudo iiod
* Then assuming iiod is running on 192.168.1.127 run epson-iiostream with params:
* > sudo ./epson-iiostream -n 192.168.1.127
* or with extra params:
* > sudo ./epson-iiostream -d epsonG354PDH0 -t epsonG354PDH0-dev0 -c 10 -n 192.168.1.127
*
* Another way to run epson-iiostream with network context is via IIOD_REMOTE.
* Set the IIOD_REMOTE environmental variable:
* > export IIOD_REMOTE=IP address of the remote platform
*
* After that run epson-iiostream with ip address of iiod server
* > sudo ./epson-iiostream -d epsonG354PDH0 -t epsonG354PDH0-dev0 -c 10
*/
...
{
    sprintf( strtmp, "%05llu", rawdata & 0xffff );
...
void finish_log( void )
{
    if ( fptr_log_file ) {
        ...
        if ( err < 0 ) {
            perror("Failed to read sampling_frequency\n");
        }
        ...
        if ( err < 0 ) {
            perror("Failed to read filter_low_pass_3db_frequency\n");
        } else if ( filter < 8 ) {
...
/* cleanup and exit */
#define DESTROY_BUFFER		1
#define DISASSOCIATE_TRIGGER	2
#define DESTROY_CONTEXT		4
static int shutdown_flags = DESTROY_BUFFER | DESTROY_CONTEXT;
static void shutdown()
{
    ...
    if (shutdown_flags & DESTROY_BUFFER) {
        printf("* Destroying buffer\n");
        if (rxbuf) iio_buffer_destroy(rxbuf);
    }

    if (shutdown_flags & DISASSOCIATE_TRIGGER) {
    ...
    if (shutdown_flags & DESTROY_CONTEXT) {
...
static void usage(__notused int argc, char *argv[])
{
    ...
    printf("  -f\tflp_3db_frequency (default as is)\n");
    printf("  -n\tremote host (default as is)\n");
    printf("  -r\ttoggles destroying buffer at exit (default as is %i)\n", !!(shutdown_flags & DESTROY_BUFFER) );
    printf("  -g\ttoggles disassosiating trigger at exit (default as is %i)\n", !!(shutdown_flags & DISASSOCIATE_TRIGGER) );
    printf("  -x\ttoggles destroying context at exit (default as is %i)\n", !!(shutdown_flags & DESTROY_CONTEXT) );
...
static void parse_options(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, ":xgrd:t:b:c:h:s:e:f:l:n:")) != -1) {
        switch (c)
        {
        case 'r':
                shutdown_flags ^= DESTROY_BUFFER;
...
int main (int argc, char **argv)
{
    ...
    if ( err < 0 ) {
        perror("Failed to read product_id\n");
        shutdown();
    }
    printf("* product_id: %s\n", product_id );
    ...
    if ( err < 0 ) {
        perror("Failed to read serial_number\n");
        shutdown();
    }
    ...
    if ( err < 0 ) {
        perror("Failed to read firmware_version\n");
        shutdown();
    }
    ...
    if ( temp_ch.chn != NULL ) {
        if( iio_channel_attr_read_double(temp_ch.chn, "offset", &in_tempX_offset) ) {
            perror("Failed to read in_temp0_offset");
            shutdown();
        }
        if( iio_channel_attr_read_double(temp_ch.chn, "scale", &in_tempX_scale) ) {
            perror("Failed to read in_temp0_scale");
            shutdown();
        }
        in_tempX_scale /= 1000.0f;
    } else {
        perror("Failed not read in_temp0 channel");
        shutdown();
    }
    ...
    if ( accel_ch_x.chn != NULL ) {
        if( iio_channel_attr_read_double(accel_ch_x.chn, "scale", &in_accel_scale) ) {
            perror("Failed to read in_accel_scale");
            shutdown();
        }
    } else {
        perror("Failed to in_accel_scale");
        shutdown();
    }
    ...
    if ( anglvel_ch_x.chn != NULL ) {
        if( iio_channel_attr_read_double(anglvel_ch_x.chn, "scale", &in_anglvel_scale) ) {
            perror("Failed to read in_anglvel_scale");
            shutdown();
        }
    } else {
        perror("Failed to read in_anglvel_scale");
    ...
    if ( sample_rate != NULL ) {
        ...
        if ( err < 0 ) {
            perror("Failed to set sampling_frequency");
        }
    }
    ...
    if ( filter_sel != NULL ) {
        err = iio_device_attr_write(dev, "filter_low_pass_3db_frequency", filter_sel);
        if ( err < 0 ) {
            perror("Failed to set filter_low_pass_3db_frequency");
            shutdown();
        }
    }
    err = iio_device_attr_read(dev, "filter_low_pass_3db_frequency", strtmp, STRTMP_SIZE);
    if ( err < 0 ) {
        perror("Failed to read filter_low_pass_3db_frequency\n");
        shutdown();
    } else {
